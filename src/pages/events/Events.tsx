...
interface EventType {
  ...
  userId: string;
  color: string;
  developer: string; // Add a developer field to store selected developer username
}
...
const UpcomingEvents: React.FC = () => {
  ...
  const { data: user } = useGetIdentity<{ email: string }>();
  const [userId, setUserId] = useState<string | null>(null);
  const [developers, setDevelopers] = useState<string[]>([]); // State to store developer usernames
  ...
  useEffect(() => {
    ...
    // Fetch list of developers (usernames) from Firebase (or any other source)
    const fetchDevelopers = async () => {
      try {
        const developersCollection = query(collection(db, "users"));
        const developersSnapshot = await getDocs(developersCollection);
        const developerList: string[] = developersSnapshot.docs.map(
    ...
    fetchDevelopers(); // Fetch developers when component mounts
  }, []);
  ...
  const fetchEvents = async (userId: string) => {
    ...
    try {
      const q = query(collection(db, "events"), where("userId", "==", userId));
      const querySnapshot = await getDocs(q);
      const filteredEvents: EventType[] = querySnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      })) as EventType[];
            ...
            <Form form={form} layout="vertical">
              ...
              >
                <Select
                  ...
                  style={{ width: "100%" }} // Ensures the dropdown fits properly
                  dropdownStyle={{ padding: 0 }} // Removes extra padding in the dropdown
                />
              </Form.Item>
            </Form>
          </Modal>
